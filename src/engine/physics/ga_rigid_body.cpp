...
void ga_rigid_body::make_not_static()
{
	_flags &= ~k_static;
}
...
void ga_rigid_body::make_not_weightless()
{
	_flags &= ~k_weightless;
}
...
void ga_rigid_body::reset_speeds()
{
	_velocity = ga_vec3f::zero_vector();
	_angular_momentum = ga_vec3f::zero_vector();
	_angular_velocity = ga_vec3f::zero_vector();
}