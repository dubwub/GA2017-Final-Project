...
bool separating_axis_test(const ga_shape* a, const ga_mat4f& transform_a, const ga_shape* b, const ga_mat4f& transform_b, ga_collision_info* info)
{
	...
	// assemble axes
	for (int i = 0; i < 3; i++) // push back the regular 6 face vectors
	{
		axes.push_back(oobb_a._half_vectors[i]);
	}
	for (int i = 0; i < 3; i++)
	{
		axes.push_back(oobb_b._half_vectors[i]);
	}

	for (int i = 0; i < 3; i++) // push back the 9 cross products of half vectors
	{
		for (int j = 0; j < 3; j++) {
			axes.push_back(ga_vec3f(ga_vec3f_cross(oobb_a._half_vectors[i], oobb_b._half_vectors[j])));
		}
	}

	collision = true;
	// project bounding boxes, check for overlap
	for (int i = 0; i < axes.size(); i++)
	{
		float radiusA = 0.0f; // calculate radius of oobb_a projected onto axis
		for (int j = 0; j < 3; j++)
		{
			radiusA += oobb_a._half_vectors[j].project_onto_abs(axes[i]).mag();
		}
		float radiusB = 0.0f; // do same for b
		for (int j = 0; j < 3; j++)
		{
			radiusB += oobb_b._half_vectors[j].project_onto_abs(axes[i]).mag();
		}
		float projACenter = oobb_a._center.dot(axes[i]); // calculate centers
		float projBCenter = oobb_b._center.dot(axes[i]);
		float minA = projACenter - radiusA; // calculate min/max of each projection
		float minB = projBCenter - radiusB;
		float maxA = projACenter + radiusA;
		float maxB = projBCenter + radiusB;
		bool overlap = !(minA > maxB || maxA < minB); // check for overlap
		if (!overlap) // we found an axis that doesn't work, a single false ruins entire collision
		{
			collision = false;
		}
		else
		{
			float penetration = ga_min((maxB - minA), (maxA - minB));
